	...
	private doPickAndOpen(options: INativeOpenDialogOptions, pickFolders: boolean, pickFiles: boolean): void {
		...
		if (!options.dialogOptions.title) {
			if (pickFolders && pickFiles) {
				options.dialogOptions.title = localize('open', "Open");
			} else if (pickFolders) {
				options.dialogOptions.title = localize('openFolder', "Open Folder");
			} else {
				options.dialogOptions.title = localize('openFile', "Open File");
			}
		}
