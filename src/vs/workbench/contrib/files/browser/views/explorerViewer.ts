...
export class ExplorerDataSource implements IAsyncDataSource<ExplorerItem | ExplorerItem[], ExplorerItem> {
	...
	constructor(
		...
		this.onConfigurationChange(this.configurationService.getValue<IFilesConfiguration>());
		this.configurationService.onDidChangeConfiguration(e => this.onConfigurationChange(this.configurationService.getValue<IFilesConfiguration>()));
	}

	private onConfigurationChange(configuration: IFilesConfiguration): void {
		//console.error('onConfigurationChange', configuration);
		...
		if (newNestingRules) {
			Object.keys(newNestingRules).forEach(element => {
				//console.error('setting nesting rules', element, newNestingRules[element]);
	...
	getChildren(element: ExplorerItem | ExplorerItem[]): Promise<ExplorerItem[]> {
		...
		const promise = element.fetchChildren(sortOrder).then((children) => {
			if (!this.nestingEnabled) {
				//console.error('nesting disabled in fetchChildren')
			...
			if (element.isVirtualDirectory && element.parent) {
				...
				//console.log('not virtual directory', element);
				return items;
			}
				...
				this.nestingConfig.forEach((childPatterns, parentPattern) => {
					...
					if (parentName) {
						childPatterns.forEach((pattern) => {
							...
							//console.log('nestingPatterns', [parentName.key, childPattern]);
						});
						...
						//console.log('virtualDirectories', [parentName.key, child]);
					}
				});
			});
